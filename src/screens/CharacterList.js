...
export default function CharacterList({
	...
	const handleRename = async () => {
		...
		if (!trimmed) return;
		const updated = await patchCharacter(selectedCharRef.current.id, {
			name: trimmed,
		});
		setCharacters((prev) =>
		...
		<SafeAreaView style={sharedStyles.screen}>
			...
			<View style={styles.header}>
				...
				<Text style={styles.headerHint}>
					Hold a character to rename or delete
				</Text>
			</View>
					...
					>
						...
						<View style={styles.cardBody}>
							...
							<Text style={styles.characterSub}>
								...
								{" · "}
								{item.classId
									? item.classId.charAt(0).toUpperCase() +
										item.classId.slice(1)
									: "Pugilist"}
								{item.subclassId
									? " · " +
										item.subclassId
						...
						<Text style={styles.actionSub}>
							...
							{selectedCharRef.current?.classId
								? selectedCharRef.current.classId
										.charAt(0)
										.toUpperCase() +
									selectedCharRef.current.classId.slice(1)
								: "Adventurer"}
